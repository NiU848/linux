...
static int mcp251xfd_open(struct net_device *ndev)
{
	...
	if (err)
		goto out_free_irq;
	//added this
	if (READ_ONCE(priv->pa_enabled))
	{
	    WRITE_ONCE(priv->pa_need_apply, true);   /* harmless if already true */
...
static int mcp251xfd_probe(struct spi_device *spi)
{
	...
	priv->can.ctrlmode_supported = CAN_CTRLMODE_LOOPBACK |
		...
		CAN_CTRLMODE_CC_LEN8_DLC | CAN_CTRLMODE_ONE_SHOT | 
	...
	//added this
	INIT_WORK(&priv->tx_ctrl_work, mcp251xfd_tx_ctrl_work);
	mutex_init(&priv->conf_lock);
	priv->pa_enabled = !!(priv->can.ctrlmode & CAN_CTRLMODE_PRESUME_ACK);
	//------------
